...
### 列印 / 另存 PDF（瀏覽器 Ctrl+P）
- 無外部函式庫；靠 `@media print` CSS（theme.css 色票 + layout.css 結構）
- 列印時：隱藏側邊欄/頂欄/篩選鈕/分頁鈕、主內容佔滿整頁、展開捲動容器讓表格完整印出（不被裁切）、卡片盡量不跨頁
- **不論當前主題（含黑夜模式）一律強制淺色**（白底深字、省墨好讀）
...
- **兩種模式**：
  - **全部員工（員編留空）**：彙總卡（員工數/出勤人日/總工時/總人力成本）+ 員工出勤彙總表（員編/課別/出勤天數/總工時/人力成本），**點任一列鑽進該員明細**；彙總表超過 50 筆時**前端分頁**（頁碼 + 上一頁/下一頁，`attendanceSetPage()`），篩選變動自動回第 1 頁；**點欄位標頭可排序**（升/降冪切換，顯示 ▲▼，`attendanceSortBy()`；員編/課別文字欄數字感知，天數/工時/成本數值排序），排序後回第 1 頁
